...
리뷰·일반 답변·타임라인의 기본 모델은 Terra, 분류명·짧은 요약은 Luna입니다.
GPT-6 Astra도 작업별로 선택할 수 있습니다. 현재 환경의 긴 본문 검증에서 시간 초과가 확인되어 기본 적용은 보류했습니다.
Codex CLI와 Python은 업데이트 후 호환성을 검증해 사용하며, 기존 리뷰는 자동으로 덮어쓰지 않습니다.
운영 설정과 업데이트 절차는 [운영 문서](docs/operations.md#모델도구-업데이트)를 참고하세요.
