"""Unavailable secondary capability boundary: doctor."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    _ = parser.add_argument(
        "--codex-canary",
        action="store_true",
        help="verify the saved-auth local generation boundary without writing",
    )
    arguments = parser.parse_args(argv)
    if arguments.codex_canary:
        return _capability_cli(CAPABILITY_NAME, ["--capability-probe"])
    return 0
